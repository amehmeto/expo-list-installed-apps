...
fun getBase64Image(context: Context, appInfo: ApplicationInfo): String {
    try {
        val icon = appInfo.loadIcon(context.packageManager)

        val packageName = appInfo.loadLabel(context.getPackageManager()).toString()
    ...
    Function("listInstalledApps") {
        try {
            ...
            for (packageInfo in pkgAppsList) {
                ...
                val iconBase64 = getBase64Image(context, appInfo)

                Log.d("ListInstalledAppsModule", "Generated iconBase64: $iconBase64")
                appList.add(mapOf(
